...
class PdfBookExportRequestHooks {
	...
	public static function parserInit( $parser ) {
		$parser->setFunctionHook( 'pdfBookExportButton', array('PdfBookExportRequestHooks', 'addButtonParser' ));
		return true;
	}
	...
	public static function addButtonParser( $input, $type = 'top', $number = 4 ) {
		$out = '<li id="ca-pdfexport"><a href="/w//index.php?action=pdfbookexport&amp;format=single">Export PDF</a></li>';
	...
	public static function onUnknownAction( $action, $article ) {
		...
		if( $action == 'pdfbookexport' ) {
			...
			function get_data($url) {
			    ...
			    $data = curl_exec($ch);
			    if(curl_errno($ch)){ echo 'Request Error:' . curl_error($ch);}
			    curl_close($ch);
			    return $data;
			}
			
			function get_group_pages($groupName) {
			    $list = "";
                $html = get_data('http://dokit/w/index.php/Special:ReorderGroup?groupName=group:'.$groupName);
                while(($html = strstr($html, 'grabbable')) != false) {
                    
                    $html = strstr($html, '>');
                    $html = substr($html, 1);
                    
                        $tmp = strstr($html, '<', true);
			            $list .= $tmp;
			            $list .= ',';
			        }
			    $list = rtrim($list, (', '));
